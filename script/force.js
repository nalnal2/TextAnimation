...
socket.on("animation prepared", function (data) {
    ...
    //     // }
    //     setTimeout(function () {
    //         // console.log("redrawGraph_ani("+i+")");
    //         redrawGraph_ani(1, i);
    //         i += 5;
    //         if (i <= tail-unit) {
    //             myLoop();
    //         } else {
    //             console.log("out of range: i=" + i);
    //         }
    //     }, 3000);
    // }
    // myLoop();
...
socket.on("animation2 prepared", function (data) {
    console.log("animation2 prepared");
    // var times = 10; // data를 이용하는 것으로 수정 필요, python 출력 필요
    // var i = 0; i *= 1;
    // console.log("tail-unit: " + (tail-unit)); // num. of last graph = tail - unit
    // function myLoop() {
...
$(window).load(function () {
    ...
    socket.emit("request static-graph"); console.log("request static-graph");
...
function showAnimation2 () {
    console.log("request animation2 par.: " + [head, tail, unit]);
    socket.emit("request animation", [head, tail, unit]); console.log("request animation2");
}
...
function redrawGraph_ani(version, graph_num) {
    setData();
    d3.selectAll("svg").remove();
    showAni(version, graph_num);
    document.getElementById("nonsingle-vertex-num").value = vertex_n;
    setTimeout(function () {
        showGraphInfo();
    }, 100);
...
// 그래프를 그리는데 필요한 변수 설정 및 실제로 그래프 그리기
function draw(_graph, force, g) {
    // 그래프의 source와 target을 node의 id를 받아와서 그릴 수 있도록 작성
    var nodeById = d3.map();
...
function showGraph() {
    ...
    setTimeout(function() {
        ...
        if (drawingMethod[1].checked) { // mst mode
            _graph = limit_node_num(_graph, nodeIDmax / 2);
            TreeMaking(_graph);
        } else { // graph mode
            single_vertex_num = 0;
            // 노드 개수 제한
            _graph = limit_node_num(_graph, nodeNLimit);
            // 노드 별 링크 개수 제한
            _graph = limit_link_num(_graph, linkNLimit);
            // 해당 노드들 중에서 single out(연결된 링크 개수가 0인 것)을 처리한다.
            singleOutPermission(_graph, singleOutChecked);
        }
        // 노드 크기를 big, medium, small로 나누고 이에 따라 실제 그려지는 크기 부여
        changeSizeByGrade(_graph);

        vertex_n = _graph.nodes.length;
        edge_n = _graph.links.length;

        draw(_graph, force, g);
    }, 500);
}

function showAni(version, graph_num) {
    var jsonpath;
    ...
    if (version == 1) {
        jsonpath = "/graph/" + userfile + "/2.1/" + userfile + "_graph_" + graph_num + ".js";
    } else if (version == 2) {
        jsonpath = "/graph/" + userfile + "/" + userfile + "_graph_" + graph_num + ".js";
    }
...
function setData() {
    ...
    switch (whole_linkNLimit) {
        case '1':
            ...
            // whole_linkNLimit = nodeNLimit / 20; // link 개수: 20
            break;
        case '2':
            whole_linkNLimit = Math.floor(nodeNLimit * 1.5);
            // whole_linkNLimit = nodeNLimit / 40; // link 개수: 10
            break;
        case '3':
            whole_linkNLimit = nodeNLimit * 2;
            // whole_linkNLimit = nodeNLimit / 80; // link 개수: 5
            break;
    }
...
function showSRange(numOfLine) {
    ...
    $("#slider-range").slider({
        ...
        slide: function (event, ui) {
            // $("#amount").val("line " + ui.values[0] + " - line " + ui.values[1]);
            head = ui.values[0], tail = ui.values[1];
            $("#amount1").val(head);
            $("#amount2").val(tail);
        }
    });
    // $("#amount").val("line " + $("#slider-range").slider("values", 0) + " - line " + $("#slider-range").slider("values", 1));
