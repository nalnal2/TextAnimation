...
function setData () {
    ...
    nodeNLimit = target1.options[target1.selectedIndex].value;
...
function find_created (prev, cur) {
    ...
    prevNodes_id_position = [];
    for (i = 0; i < prev.nodes.length; ++i) {
        prevNodes_id.push(prev.nodes[i].id);
        prevNodes_id_position.push({id: prev.nodes[i].id, x: prev.nodes[i].x, y: prev.nodes[i].y});
    }
    ...
    cur.nodes.forEach(function (node) {
        ...
        if ((index = prevNodes_id.indexOf(node.id)) == -1) {
            ...
            if (node.size != prev.nodes[index].size) {
                changes.sizeChangedNodes.push({id: node.id, newSize: node.size});
            }
        }
    });
...
function make_graphToDraw () {
    if (graph_num == head) {
        ...
        d3.selectAll("circle").style("fill", "#000000");
    ...
    // graph_toDraw의 node중에 prevNodes_id_position에 존재하는 node는 x, y를 설정한다.
    prevNodes_id_position.forEach(function (prevNode) {
        for (var i = 0; i < graph_toDraw.nodes.length; ++i) {
            if (graph_toDraw.nodes[i].id == prevNode.id) {
                graph_toDraw.nodes[i].x = prevNode.x;
                graph_toDraw.nodes[i].y = prevNode.y;
                console.log('maintained position node: ');
                console.log(graph_toDraw.nodes[i].id);
                console.log(graph_toDraw.nodes[i].x);
                console.log(graph_toDraw.nodes[i].y);
            }
        }
    });
    ...
    changes.createdNodes.forEach(function (node) {
        d3.select($("#"+node.id)[0]).style("fill", "#5e5e5e");
    });
    changes.disappearedNodes.forEach(function (nodeID) {
...
function restart () {
    ...
    node = node.enter()
            ...
            .call(function (node) { node.transition().attr("r", function (d) { return d.size; }); })
            .attr("id", function (d) { return d.id; })
            .attr("cx", function (d) { if (d.x) { return d.x; }})
            .attr("cy", function (d) { if (d.y) { return d.y; }})
            .style("stroke", "#4f97ba")
            .style("fill", function (d) {
                if (d.type == NORMAL) {
                    return "#000000";
